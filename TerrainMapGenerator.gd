...
@export var mod : float = snappedf(castlewidth, tile_size.x) / tile_size.x
	...
	for x in world_tiles_x:
		...
		for y in world_tiles_y:
			...
			if y > current_displacement and y < world_tiles_y-2:
				...
				rdm = ceil(rdm)
				rdm = int(rdm)
				set_cell(0, Vector2i(x,y), rdm)
			elif y >= world_tiles_y-2:
				set_cell(0, Vector2i(x,y), 7)
			# End Y Loop
